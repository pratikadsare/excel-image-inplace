...
# HEAD/GET with graceful fallbacks
def get_content_type(url: str, timeout: float = 10.0) -> Optional[str]:
    try:
        ...
        pass
    # Some CDNs don’t support HEAD reliably; try GET with stream to avoid big downloads
    try:
...
def adjust_dimensions(ws, col_indices:Set[int], row_height_px:int):
    # Column width roughly square to image height; set row height for all rows (consistent grid look)
...
def place_anchor_image(ws, cell, url: str, w_px: int, h_px: int, keep_note: bool):
    # Actual image embed; works on all Excels
...
try:
    if uploaded:
        ...
        # Read headers from first target sheet
        ...
        # Count totals to build a progress target
        total_urls = 0
        preview_rows = []
        ...
        if st.button("Process & Prepare Download", type="primary"):
            ...
            for s in target_sheets:
                ...
                for cell in iter_target_cells(ws, preview_targets, header_row=header_row):
                    ...
                    # Check content type first; skip PDFs and any non-images
                    ct = get_content_type(url)
                    if not is_image_content_type(ct):
                        # Mark skipped with a non-intrusive comment
                        try:
                            note = f"Skipped (non-image: {ct or 'unknown'})\n{url}"
                            if keep_notes and not create_adjacent:
                                cell.comment = Comment(note, "PreviewBot")
                        except Exception:
                            pass
                        skipped_nonimage += 1
                        processed += 1
                        if total_urls:
                            ...
                            status.write(f"Processed {processed} of {total_urls} | inserted: {inserted}, skipped: {skipped_nonimage}, failed: {failed}")
                        continue
            ...
            # Save to buffer + dynamic filename
            out = io.BytesIO()
            ...
            status.write(f"Completed {processed} of {total_urls} | inserted: {inserted}, skipped: {skipped_nonimage}, failed: {failed}")
            st.success("Done! Download your updated workbook below.")
            st.download_button("⬇️ Download updated masterfile", data=out, file_name=out_name,
