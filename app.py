...
if uploaded:
    ...
    selected_by_name=st.multiselect("Pick URL columns by header (auto-detected suggestions pre-selected)",
                                    options=headers,default=auto_cols_names)
    ...
    if st.button("Process & Prepare Download",type="primary"):
        ...
        for s in target_sheets:
            ...
            if create_adjacent:
                ...
                for col_idx in sorted(targets):
                    ...
                    base=ws.cell(row=header_row,column=col_idx+inserted).value
            ...
            for c in preview_targets:
                ws.column_dimensions[get_column_letter(c)].width=px_to_col_width(max(width,height))
            target_h_pt=px_to_row_height(max(width,height))
            for r in range(1,ws.max_row+1):
                ws.row_dimensions[r].height=target_h_pt
            ...
            for cell in iter_target_cells(ws,preview_targets,header_row=header_row):
                ...
                try:
                    ...
                    if total_urls>0:
                        ...
                        status.write(f"Processed {processed} of {total_urls} URLs")
        ...
        if orig_name.lower().endswith(".xlsx"):
            out_name=orig_name[:-5]+"-preview.xlsx"
        else:
            out_name=orig_name+"-preview.xlsx"

        status.write(f"Completed {processed} of {total_urls} URLs")
